/**
 * API Route: Upload Article Image
 * 
 * Maneja la subida de imágenes para artículos
...
export async function POST(request: Request) {
  try {
    ...
    if (previousImageUrl && previousImageUrl.startsWith('/images/articles/')) {
      try {
    ...
    const uploadDir = join(process.cwd(), "public", "images", "articles")
    ...
    logger.info('Imagen de artículo subida', { fileName, size: file.size })
    ...
    logger.error('Error subiendo imagen de artículo', error)
    return NextResponse.json(
